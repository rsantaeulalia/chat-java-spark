...
import com.asapp.backend.challenge.model.adapter.MessageAdapterTest;
...
public class MessagesControllerTest {
    ...
    public MessagesControllerTest(final MessageService messageService) {
    ...
    public Route sendMessage = (Request req, Response resp) -> {
        ...
        Message message = messageService.saveMessage(MessageAdapterTest.toDomain(messageRequest));
        resp.status(201);
        return JSONUtil.dataToJson(MessageAdapterTest.toMessageResponse(message));
    };
    ...
    public Route getMessages = (Request req, Response rep) -> {
        ...
        return JSONUtil.dataToJson(messages.stream().map(MessageAdapterTest::toApi).collect(toList()));
    };
